...
class OperatorResponsePresentationContractTests(unittest.TestCase):
    ...
    def test_response_band_uses_one_operator_facing_taxonomy(self) -> None:
        ...
        for label in ("none", "light", "normal", "hard"):
            self.assertIn(f"<span>{label}</span>", labels)
        for legacy_label in ("small", "moderate", "large"):
            self.assertNotIn(f"<span>{legacy_label}</span>", labels)
        self.assertIn(
            '${levelLabel(responseBandLevel)} · ${formatPercent(peak, 0)} response',
            app_js,
        )
        self.assertNotIn(
            '${levelLabel(responseBandLevel)} · ${activeModelDisplayName',
            app_js,
        )
