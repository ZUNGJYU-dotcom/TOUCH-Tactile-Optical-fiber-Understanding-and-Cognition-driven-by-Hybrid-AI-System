"""Shared temporal-window transformations used by training and live shadow inference."""
...
def temporal_summary_features(values: np.ndarray) -> np.ndarray:
    """Summarize ``[windows, time, features]`` without using labels or future data."""

    array = np.asarray(values, dtype=np.float32)
    if array.ndim != 3 or array.shape[1] < 2:
        raise ValueError("dynamic values must have shape [windows, time>=2, features]")
